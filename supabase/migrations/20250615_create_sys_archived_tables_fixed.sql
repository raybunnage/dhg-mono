...
-- Add indexes
CREATE INDEX IF NOT EXISTS idx_sys_archived_tables_table_name ON sys_archived_tables(table_name);
CREATE INDEX IF NOT EXISTS idx_sys_archived_tables_archived_at ON sys_archived_tables(archived_at);

-- Add RLS policies
ALTER TABLE sys_archived_tables ENABLE ROW LEVEL SECURITY;

-- Allow authenticated users to read
DROP POLICY IF EXISTS "Allow authenticated read" ON sys_archived_tables;
CREATE POLICY "Allow authenticated read" ON sys_archived_tables
  FOR SELECT TO authenticated
  USING (true);

-- Only service role can insert/update/delete
DROP POLICY IF EXISTS "Service role all access" ON sys_archived_tables;
CREATE POLICY "Service role all access" ON sys_archived_tables
  FOR ALL TO service_role
  USING (true);

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_table_ddl(TEXT);
DROP FUNCTION IF EXISTS get_table_foreign_keys(TEXT);
