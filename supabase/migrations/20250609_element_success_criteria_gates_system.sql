...
-- Ties success criteria and quality gates to granular elements (app features, CLI commands, services)

-- Success criteria tied to elements (not tasks)
CREATE TABLE IF NOT EXISTS element_success_criteria (
  ...
  criteria_type TEXT NOT NULL CHECK (criteria_type IN ('functional', 'performance', 'security', 'ux', 'integration', 'documentation', 'testing')),
  ...
  parent_criteria_id UUID REFERENCES element_success_criteria(id) ON DELETE CASCADE,
...
CREATE INDEX idx_element_success_criteria_element ON element_success_criteria(element_type, element_id);
CREATE INDEX idx_element_success_criteria_type ON element_success_criteria(criteria_type);
CREATE INDEX idx_element_quality_gates_element ON element_quality_gates(element_type, element_id);
CREATE INDEX idx_element_quality_gates_type ON element_quality_gates(gate_type);
CREATE INDEX idx_task_criteria_inheritance_task ON task_criteria_inheritance(task_id);
...
-- Read access for all
CREATE POLICY "element_success_criteria_read_all" ON element_success_criteria
...
CREATE POLICY "element_success_criteria_write_authenticated" ON element_success_criteria
...
COMMENT ON TABLE element_success_criteria IS 'Reusable success criteria tied to specific elements (features, commands, services)';
