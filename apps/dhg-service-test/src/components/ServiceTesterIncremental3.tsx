...
export function ServiceTesterIncremental3() {
  const [results, setResults] = useState<ServiceTestResult[]>([]);
  const [selectedGroups, setSelectedGroups] = useState<Set<string>>(
    new Set(['Core Services', 'Feature Services', 'Document Services', 'Element Services'])
  );
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-8">Service Tester (+ Element Services)</h1>
      ...
      <div className="bg-yellow-50 p-4 rounded-lg">
        ...
        <p className="text-sm mb-2">
          Now testing Element Services (ElementCatalogService, ElementCriteriaService) in addition to Core, Feature, and Document Services.
        </p>
        <p className="text-sm">
          If this fails, the issue is likely in one of the element services or their dependencies.
        </p>
      </div>
    </div>
  );
