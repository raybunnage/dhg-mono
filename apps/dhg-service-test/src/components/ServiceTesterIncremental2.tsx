...
export function ServiceTesterIncremental2() {
  const [results, setResults] = useState<ServiceTestResult[]>([]);
  const [selectedGroups, setSelectedGroups] = useState<Set<string>>(
    new Set(['Core Services', 'Feature Services', 'Document Services'])
  );
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-8">Service Tester (Core + Feature + Document Services)</h1>
      ...
      <div className="bg-yellow-50 p-4 rounded-lg">
        ...
        <p className="text-sm mb-2">
          Now testing Document Services (DocumentTypeService, DocumentClassificationService) in addition to Core and Feature Services.
        </p>
        <p className="text-sm">
          If this fails, the issue is likely in one of the document services or their dependencies.
        </p>
      </div>
    </div>
  );
