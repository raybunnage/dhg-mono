...
export const ProfileSettingsPage: React.FC = () => {
  ...
  console.log('[ProfileSettingsPage] Rendering with user:', user);
  console.log('[ProfileSettingsPage] localStorage dhg_auth_user:', localStorage.getItem('dhg_auth_user'));

  useEffect(() => {
    console.log('[ProfileSettingsPage] useEffect - user:', user);
    if (!user) {
      console.log('[ProfileSettingsPage] No user detected, checking localStorage...');
      const storedUser = localStorage.getItem('dhg_auth_user');
      if (!storedUser) {
        console.log('[ProfileSettingsPage] No user in localStorage either, will redirect after delay');
        // Give auth state a chance to initialize
        const timer = setTimeout(() => {
          const currentUser = localStorage.getItem('dhg_auth_user');
          if (!currentUser) {
            console.log('[ProfileSettingsPage] Still no user after wait, redirecting to home');
            navigate('/');
          }
        }, 500);
        return () => clearTimeout(timer);
      } else {
        console.log('[ProfileSettingsPage] User exists in localStorage but not in hook yet, waiting...');
        return;
      }
    }
  ...
  const loadProfile = async () => {
    ...
    console.log('[ProfileSettingsPage] Loading profile for user:', user.id);
    
    try {
      setIsLoading(true);
      const result = await profileService.getProfile(user.id);
      console.log('[ProfileSettingsPage] Profile load result:', result);
      
      if (result.success && result.data) {
        console.log('[ProfileSettingsPage] Profile data loaded:', result.data);
        setProfileData(result.data);
      } else {
        // No profile yet - show form to complete it
        console.log('[ProfileSettingsPage] No profile found, showing edit form');
        setIsEditing(true);
      }
    } catch (err) {
      console.error('[ProfileSettingsPage] Error loading profile:', err);
  ...
  const handleProfileSubmit = async (formData: ProfileFormData) => {
    ...
    console.log('[ProfileSettingsPage] handleProfileSubmit called with data:', formData);
    console.log('[ProfileSettingsPage] User ID:', user.id);
    
    setIsLoading(true);
    setError('');

    try {
      const result = await profileService.saveProfile(user.id, formData);
      console.log('[ProfileSettingsPage] Profile save result:', result);
      ...
      console.error('[ProfileSettingsPage] Profile update error:', err);
