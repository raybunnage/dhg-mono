...
export const TrackedAudioPlayer = ({ 
  url, 
  title, 
  mediaId,
  onTimeUpdate, 
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // Track seek if there's a significant jump
      const timeDiff = Math.abs(newTime - lastReportedTime.current);
      if (timeDiff > 2 && !isPlaying) {
        tracking.trackSeek(lastReportedTime.current, newTime);
      }
    ...
    const handleEnded = () => {
      setIsPlaying(false);
      tracking.trackPause(audio.currentTime);
      
      // Calculate completion percentage
      const completion = duration > 0 ? (audio.currentTime / duration) * 100 : 0;
  ...
  }, [onTimeUpdate, title, isPlaying, duration, tracking]);
  
  // Clean up tracking session on unmount
  useEffect(() => {
    return () => {
      if (isPlaying) {
  ...
  const togglePlay = async () => {
    ...
    if (audioRef.current) {
      if (isPlaying) {
        ...
        setIsPlaying(false);
        await tracking.trackPause(audioRef.current.currentTime);
      } else {
  ...
  const handleSeek = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (audioRef.current) {
      ...
      tracking.trackSeek(previousTime, seekTime);
    }
  };
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (audioRef.current) {
      ...
      tracking.trackVolumeChange(audioRef.current.currentTime, newVolume);
    }
  };
  ...
  const handlePlaybackRateChange = (rate: number) => {
    ...
    if (audioRef.current) {
      ...
      tracking.trackSpeedChange(audioRef.current.currentTime, rate);
    }
  };
  ...
  const handleAddBookmark = async () => {
    ...
    if (note !== null) { // User didn't cancel
      await tracking.addBookmark(currentTime, note || undefined, 'manual');
