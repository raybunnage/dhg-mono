...
CLI_PIPELINE_DIR="$SCRIPT_DIR"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"
MONO_ROOT="$(cd "$ROOT_DIR/.." && pwd)"
CLI_DIR="$MONO_ROOT/packages/cli"  # Updated to use packages/cli directory

# Define ABSOLUTE paths for output directories to be within cli-pipeline instead of root
SCAN_OUTPUT="$CLI_PIPELINE_DIR/script-scan-results.json"
ANALYSIS_DIR="$CLI_PIPELINE_DIR/script-analysis-results"

# Print directory information for debugging
echo "CLI Pipeline directory: $CLI_PIPELINE_DIR"
...
# Use absolute paths and don't change directory
