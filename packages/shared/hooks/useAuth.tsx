/**
 * Shared React Authentication Hooks
 * 
 * Provides reusable authentication hooks for React components across applications
 */
    ...
    try {
      ...
      if (result.error) {
        setState(prev => ({
          ...prev,
          loading: false,
          error: new Error((result.error as any)?.message || 'Authentication failed')
        }));
      } else {
        setState({
          user: result.user,
  ...
  const sendMagicLink = useCallback(async (email: string): Promise<AuthResult> => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: result.error ? new Error((result.error as any)?.message || 'Authentication failed') : null
      }));
