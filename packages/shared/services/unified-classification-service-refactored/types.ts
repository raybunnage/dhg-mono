/**
 * Type definitions for the Unified Classification Service (Refactored)
 */
...
// Metrics for the UnifiedClassificationService
export interface UnifiedClassificationServiceMetrics {
  classificationsRequested: number;
...
// Configuration for the UnifiedClassificationService
export interface UnifiedClassificationServiceConfig {
...
// Service interfaces (duck typing for dependency injection)
export interface GoogleDriveServiceInterface {
  downloadFile(driveId: string, localPath: string): Promise<string>;
  exportFile(driveId: string, mimeType: string): Promise<string>;
  getHealthStatus?(): Promise<{ healthy: boolean; details: Record<string, any> }>;
}

export interface PromptServiceInterface {
  loadPrompt(promptName: string): Promise<{ success: boolean; error?: string }>;
  usePromptWithClaude(
    promptName: string, 
    userMessage: string, 
...
export interface PdfProcessorServiceInterface {
...
export interface FilterServiceInterface {
  applyFilterToQuery(query: any): Promise<any>;
}