 ...
 * providing a consistent interface with environment-specific implementations.
 * 
 * Usage:
 * - Frontend (React): Import from this file and use the same patterns as backend
 * - Backend (Node.js): This adapter automatically detects the environment and uses the appropriate implementation
 */
...
/**
 * SupabaseClientAdapter - Universal adapter that works in both environments
 */
export class SupabaseClientAdapter {
  ...
  private constructor() {
    ...
    if (this.isBrowser) {
      ...
      // This branch won't be reached in the browser (code elimination)
      ...
      if (this.supabaseKey) {
        const maskedKey = this.supabaseKey.substring(0, 5) + '...' + this.supabaseKey.substring(this.supabaseKey.length - 5);
        console.log(`Supabase adapter (Node.js): Key found ${maskedKey}`);
      } else {
        console.error('Supabase adapter (Node.js): No API key found');
      }
    }
      ...
      if (this.isBrowser) {
        ...
        this.client = createClient<Database>(
          ...
          this.supabaseKey,
          {
            ...
            global: {
              headers: {
                'X-Client-Info': 'dhg-improve-experts'
              }
            }
          }
        );
        ...
        this.client = createClient<Database>(
          ...
          {
            global: {
              // Add a custom timeout for fetch operations to handle DNS issues
              fetch: (url, init) => {
  ...
  public async getDiagnostics(): Promise<{
    ...
    const environment = this.isBrowser ? 'browser' : 'node.js';
   ...
   * For Node.js, this will always return success as we use service role key
   */
  public async ensureAuth(): Promise<{ success: boolean; diagnostics: string }> {
    // For Node.js, we're always "authenticated" via service role key
    if (!this.isBrowser) {
      return { success: true, diagnostics: "Node.js environment - using service role key" };
    }
