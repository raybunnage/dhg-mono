...
export class WorktreeSwitcherService {
  ...
  getWorktrees(): Worktree[] {
    try {
      ...
      return worktrees.map(wt => {
        ...
        let cursorPid: string | null = null;
        try {
        ...
        let peacockColor: string | null = null;
   ...
   */
  openInCursor(worktreePath: string): Promise<{ success: boolean; message: string }> {
   ...
   */
  async createWorktree(branchName: string, baseBranch: string = 'main'): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  }> {
    try {
      ...
      // Create the worktree
      execSync(`git worktree add -b ${branchName} ${worktreePath} ${baseBranch}`, { encoding: 'utf8' });
   ...
   * Remove a worktree
   */
  async removeWorktree(worktreePath: string): Promise<{
    success: boolean;
    error?: string;
  }> {
    try {
  ...
  getDefaultColor(branch?: string): string {
    if (!branch) return '#fbc02d';
    
    if (branch.includes('main')) return '#42b883';
    if (branch.includes('development')) return '#007ACC';
    if (branch.includes('improve-cli-pipelines')) return '#832561';
    if (branch.includes('feature')) return '#fd9827';
    if (branch.includes('fix') || branch.includes('bug')) return '#dd5145';
