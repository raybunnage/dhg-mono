...
exports.GitOperationsService = void 0;
...
class GitOperationsService {
    protectedBranches = ['main', 'master', 'development', 'production'];
    /**
     ...
     */
    async enhanceWorktreeInfo(worktree) {
        try {
            // Get last commit info
            const { stdout: lastCommit } = await execAsync(`cd "${worktree.path}" && git log -1 --format="%h|%s|%ar|%an" 2>/dev/null || echo ""`);
            const [commitHash, message, relativeTime, author] = lastCommit.trim().split('|');
            // Check for uncommitted changes
            const { stdout: statusOutput } = await execAsync(`cd "${worktree.path}" && git status --porcelain 2>/dev/null | wc -l || echo "0"`);
            const uncommittedChanges = parseInt(statusOutput.trim()) || 0;
            // Check if branch is ahead/behind
            const { stdout: branchStatus } = await execAsync(`cd "${worktree.path}" && git rev-list --left-right --count HEAD...@{u} 2>/dev/null || echo "0\t0"`);
    ...
    async getBranches() {
        ...
        const enhancedBranches = await Promise.all(localBranches.map(branch => this.enhanceBranchInfo(branch)));
        return {
     ...
     */
    async enhanceBranchInfo(branch) {
        try {
     ...
     */
    async cleanupBranches(branches, dryRun = true) {
        const results = [];
     ...
     */
    async getWorktreeCommits(worktreePath, limit = 50) {
        const decodedPath = decodeURIComponent(worktreePath);
        // Get commit hashes first
        const { stdout: hashesOutput } = await execAsync(`cd "${decodedPath}" && git log --format="%H" -${limit}`);
        const hashes = hashesOutput.trim().split('\n').filter(h => h);
        // Get detailed info for each commit
        const commits = await Promise.all(hashes.map(hash => this.getCommitInfo(decodedPath, hash)));
        // Get current branch name
        const { stdout: branchName } = await execAsync(`cd "${decodedPath}" && git branch --show-current`);
        return {
     ...
     */
    async getCommitInfo(worktreePath, hash) {
        try {
            // Get basic commit info
            const { stdout: commitInfo } = await execAsync(`cd "${worktreePath}" && git log --format="%s|%an|%ae|%ar|%ai" -1 ${hash}`);
            // Get full commit message to extract task ID
            const { stdout: fullMessage } = await execAsync(`cd "${worktreePath}" && git log --format="%B" -1 ${hash}`);
...
exports.GitOperationsService = GitOperationsService;
//# sourceMappingURL=GitOperationsService.js.map