...
/**
 * DeploymentService - Manages application deployments with validation and rollback capabilities
 * Implements singleton pattern for managing deployment operations
 */
export class DeploymentService {
  ...
  /**
   * Create and manage a deployment with full validation
   */
  async createDeployment(config: DeploymentConfig): Promise<DeploymentResult> {
    ...
    try {
      ...
      const failedValidations = validations.filter(
        v => v.status === 'failed' && !config.skipValidations?.includes(v.type)
      );
  ...
  /**
   * Validate tests (placeholder for now)
   */
   ...
   */
  async getDeploymentHistory(limit: number = 10): Promise<DeploymentRun[]> {
    const { data, error } = await this.supabase
      .from('deployment_runs')
      .select('*')
  ...
  async healthCheck(): Promise<boolean> {
    try {
      // Check if we can access the deployment_runs table
