#!/bin/bash
# Dev Tasks CLI Pipeline Health Check
# This script verifies that the dev tasks pipeline is properly configured

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="$SCRIPT_DIR/../../../logs"
LOG_FILE="$LOG_DIR/dev-tasks-health-check.log"
...
echo "=== Dev Tasks CLI Pipeline Health Check - $TIMESTAMP ===" > "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Main testing function
main() {
  echo "Running Dev Tasks CLI Pipeline health check..."
  ...
  if [ -f "$SCRIPT_DIR/dev-tasks-cli.sh" ]; then
    echo "✅ CLI script found: dev-tasks-cli.sh" | tee -a "$LOG_FILE"
  else
    echo "❌ FAILED: CLI script dev-tasks-cli.sh not found" | tee -a "$LOG_FILE"
    exit 1
  fi
  ...
  # Check database connectivity (dev tasks uses database)
  echo "" | tee -a "$LOG_FILE"
  ...
  echo "" | tee -a "$LOG_FILE"
  echo "✅ Dev Tasks CLI Pipeline health check completed successfully!" | tee -a "$LOG_FILE"
  return 0
}
