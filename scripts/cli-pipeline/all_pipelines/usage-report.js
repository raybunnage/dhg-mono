...
async function generateUsageReport(options) {
    ...
    try {
        ...
        const activePipelines = Object.keys(pipelines).filter(pipeline => {
            ...
            const hasRecentActivity = pipelineStats.some((stat) => stat.pipeline_name === pipeline &&
        ...
        const totalExecutions = pipelineStats.reduce((sum, stat) => sum + stat.total_executions, 0);
        const totalSuccessful = pipelineStats.reduce((sum, stat) => sum + stat.successful_executions, 0);
        const totalFailed = pipelineStats.reduce((sum, stat) => sum + stat.failed_executions, 0);
        const totalRunning = pipelineStats.reduce((sum, stat) => sum + stat.running_executions, 0);
        ...
        allCommands.sort((a, b) => b.total_executions - a.total_executions);
        // Take top N commands
        ...
        markdown += `| ------- | -------- | ---------- | ------------ | --------- |\n`;
        topCommandsOverall.forEach((stat) => {
        ...
        const recentExecutionsByCommand = {};
        ...
        activePipelines.sort((a, b) => {
            const aExecs = pipelines[a].reduce((sum, stat) => sum + stat.total_executions, 0);
            const bExecs = pipelines[b].reduce((sum, stat) => sum + stat.total_executions, 0);
        ...
        for (const pipeline of activePipelines) {
            ...
            const totalPipelineExecutions = pipelineCommands.reduce((sum, stat) => sum + stat.total_executions, 0);
            const successfulPipelineExecutions = pipelineCommands.reduce((sum, stat) => sum + stat.successful_executions, 0);
            const failedPipelineExecutions = pipelineCommands.reduce((sum, stat) => sum + stat.failed_executions, 0);
            markdown += `### ${pipeline}\n\n`;
            ...
            pipelineCommands.sort((a, b) => b.total_executions - a.total_executions);
            ...
            markdown += `| ------- | ---------- | ------------ | ------------ | --------- |\n`;
            topPipelineCommands.forEach((stat) => {
                const successRate = (stat.successful_executions / (stat.total_executions || 1) * 100).toFixed(1);
                const lastUsed = new Date(stat.last_execution).toLocaleString();
                let avgDuration = 'N/A';
