...
display_help() {
  ...
  echo ""
  echo "PROMPT EXECUTION & TRACKING:"
  echo "    execute <prompt-name>      Execute a prompt with performance tracking"
  echo "    select-prompt              Select the best prompt based on document characteristics"
  echo "    set-mappings <prompt>      Set document type and MIME type mappings for a prompt"
  echo "    stats                      View prompt execution statistics and performance metrics"
  echo ""
  ...
  echo ""
  echo "PROMPT EXECUTION:"
  echo "  # Execute a prompt with document context"
  echo "  ./prompt-service-cli.sh execute document-classification-prompt -d \"1234567890\""
  echo ""
  echo "  # Select best prompt for a PDF document"
  echo "  ./prompt-service-cli.sh select-prompt -t pdf -m application/pdf -s 100000"
  echo ""
  echo "  # Set document type mappings for a prompt"
  echo "  ./prompt-service-cli.sh set-mappings scientific-document-analysis-prompt -d pdf docx -p 10"
  echo ""
  echo "  # View execution statistics"
  echo "  ./prompt-service-cli.sh stats -l 10 -s avg-time"
  echo ""
