#!/bin/bash
# Tracking CLI Pipeline Health Check
# This script verifies that the tracking pipeline is properly configured

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="$SCRIPT_DIR/../../../logs"
LOG_FILE="$LOG_DIR/tracking-health-check.log"
...
echo "=== Tracking CLI Pipeline Health Check - $TIMESTAMP ===" > "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Main testing function
main() {
  echo "Running Tracking CLI Pipeline health check..."
  ...
  if [ -f "$SCRIPT_DIR/tracking-cli.sh" ]; then
    echo "✅ CLI script found: tracking-cli.sh" | tee -a "$LOG_FILE"
  else
    echo "❌ FAILED: CLI script tracking-cli.sh not found" | tee -a "$LOG_FILE"
    exit 1
  fi
  ...
  # Check database connectivity (tracking uses database)
  echo "" | tee -a "$LOG_FILE"
  ...
  echo "" | tee -a "$LOG_FILE"
  echo "✅ Tracking CLI Pipeline health check completed successfully!" | tee -a "$LOG_FILE"
  return 0
}
