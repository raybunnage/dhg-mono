#!/bin/bash
# Monitoring CLI Pipeline Health Check
# This script verifies that the monitoring pipeline is properly configured
...
echo "=== Monitoring CLI Pipeline Health Check - $TIMESTAMP ===" > "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Main testing function
main() {
  echo "Running Monitoring CLI Pipeline health check..."
  ...
  if [ -f "$SCRIPT_DIR/monitoring-cli.sh" ]; then
    echo "✅ CLI script found: monitoring-cli.sh" | tee -a "$LOG_FILE"
  else
    echo "❌ FAILED: CLI script monitoring-cli.sh not found" | tee -a "$LOG_FILE"
    exit 1
  fi
  ...
  # Check database connectivity (monitoring uses database)
  echo "" | tee -a "$LOG_FILE"
  ...
  echo "" | tee -a "$LOG_FILE"
  echo "✅ Monitoring CLI Pipeline health check completed successfully!" | tee -a "$LOG_FILE"
  return 0
}
