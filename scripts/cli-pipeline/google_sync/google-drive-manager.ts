  ...
  .action(async (rootId: string | undefined, options: { dryRun?: boolean; timeout?: string }) => {
    try {
      if (rootId) {
        ...
        // Just log what would happen for now - a proper implementation would reuse the sync-folder logic
        console.log(`Would execute: sync-folder ${data.drive_id} ${options.dryRun ? '--dry-run' : ''}`);
        
        // In a real implementation, you would call the sync-folder function directly
        // rather than trying to call the command
      } else {
