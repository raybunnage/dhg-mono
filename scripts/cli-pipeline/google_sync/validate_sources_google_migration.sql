...
SELECT
    (SELECT COUNT(*) FROM google_sources) AS original_count,
    (SELECT COUNT(*) FROM google_sources_new) AS new_count;

-- 2. Check if counts match
SELECT 
    (SELECT COUNT(*) FROM google_sources) = (SELECT COUNT(*) FROM google_sources_new) AS counts_match;

-- 3. Check ID integrity (all ids from original are in new table)
SELECT COUNT(*) AS missing_ids
FROM google_sources sg
LEFT JOIN google_sources_new sg2 ON sg.id = sg2.id
WHERE sg2.id IS NULL;

-- 4. Check root_drive_id improvement
SELECT 
    (SELECT COUNT(*) FROM google_sources WHERE root_drive_id IS NOT NULL AND root_drive_id != '') AS original_with_root,
    (SELECT COUNT(*) FROM google_sources_new WHERE root_drive_id IS NOT NULL AND root_drive_id != '') AS new_with_root;

-- 5. Verify path completeness
SELECT 
    COUNT(*) AS missing_paths
FROM google_sources_new 
