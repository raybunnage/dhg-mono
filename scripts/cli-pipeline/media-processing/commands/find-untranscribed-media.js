...
// Import logger and Supabase service
 ...
 */
function findFileInDirectory(filename, directory, deep = false) {
  try {
 ...
 */
async function findUntranscribedFiles() {
  ...
  const transcribedSourceIds = new Set(expertDocs.map(doc => doc.source_id));
  
  // Filter for untranscribed sources
  const untranscribedSources = sources.filter(source => 
