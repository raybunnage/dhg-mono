...
// Just for debugging - these should all match now
if (process.env.ANTHROPIC_API_KEY !== process.env.CLAUDE_API_KEY && 
    process.env.ANTHROPIC_API_KEY && process.env.CLAUDE_API_KEY) {
  console.log('⚠️ Warning: ANTHROPIC_API_KEY and CLAUDE_API_KEY are different!');
}

// Global variables for tracking success
let exitCode = 0;

/**
 * Process a single untyped script from the database
 * @returns {Promise<boolean>} True if classification was successful
 */
 ...
 * Validates configuration and initiates script classification
