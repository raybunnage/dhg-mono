#!/bin/bash
# Health check script for Scripts CLI pipeline
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/scripts-health-check.log"

# Initialize log file
echo "Scripts CLI Pipeline Health Check - $(date)" > "$LOG_FILE"
echo "==========================================" >> "$LOG_FILE"
...
echo "Running Scripts CLI Pipeline health check..."
...
run_health_check() {
  ...
  echo "✅ Scripts CLI Pipeline health check passed" | tee -a "$LOG_FILE"
