...
if [ "$1" = "help" ] || [ "$1" = "--help" ] || [ "$1" = "-h" ] || [ -z "$1" ]; then
  ...
  echo ""
  echo "COMMANDS:"
  echo "  list                         List all document types"
  echo "  get                          Get details of a document type"
  echo "  create                       Create a new document type"
  echo "  update                       Update an existing document type"
  ...
  echo "  ./document-types-cli.sh create --name \"Research Report\" --category \"Research\" --description \"Reports containing research findings\""
  echo ""
  echo "  # Generate a document type definition using AI"
  echo "  ./document-types-cli.sh generate --prompt \"Create a document type for financial reports that include quarterly revenue, expenses, and profit margins\""
  echo ""
  echo "  # Generate and create a document type in one step"
  echo "  ./document-types-cli.sh generate --prompt \"Create a document type for customer feedback surveys\" --create"
  echo ""
