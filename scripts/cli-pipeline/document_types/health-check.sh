#!/bin/bash
# Health check script for Document Types CLI pipeline
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/document-types-health-check.log"

# Initialize log file
echo "Document Types CLI Pipeline Health Check - $(date)" > "$LOG_FILE"
echo "==========================================" >> "$LOG_FILE"
...
echo "Running Document Types CLI Pipeline health check..."
  ...
  SUPABASE_TEST_RESULT=$(cd "$ROOT_DIR" && npx ts-node -e "
    ...
    async function testConnection() {
      try {
        ...
        const { data, error } = await supabase.from('document_types').select('count(*)', { count: 'exact', head: true });
        if (error) {
...
run_health_check() {
  ...
  if [ $? -ne 0 ]; then
    ...
    echo "✅ Document Types CLI Pipeline health check passed" | tee -a "$LOG_FILE"
  fi
