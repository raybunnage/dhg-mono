#!/bin/bash
# Merge CLI Pipeline Health Check
# This script verifies that the merge pipeline is properly configured

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOG_DIR="$SCRIPT_DIR/../../../logs"
LOG_FILE="$LOG_DIR/merge-health-check.log"
...
echo "=== Merge CLI Pipeline Health Check - $TIMESTAMP ===" > "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Main testing function
main() {
  echo "Running Merge CLI Pipeline health check..."
  ...
  if [ -f "$SCRIPT_DIR/merge-cli.sh" ]; then
    echo "✅ CLI script found: merge-cli.sh" | tee -a "$LOG_FILE"
  else
    echo "❌ FAILED: CLI script merge-cli.sh not found" | tee -a "$LOG_FILE"
    exit 1
  fi
  ...
  echo "" | tee -a "$LOG_FILE"
  echo "✅ Merge CLI Pipeline health check completed successfully!" | tee -a "$LOG_FILE"
  return 0
}
