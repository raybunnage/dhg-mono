...
# Test suite for mime-types-cli-refactored.sh
# Tests: help, health-check, MIME type operations

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CLI_SCRIPT="$SCRIPT_DIR/mime-types-cli-refactored.sh"
...
echo "=== Testing mime-types-cli-refactored.sh ==="
echo ""
...
# Test 2: Help command
run_test "help command" "$CLI_SCRIPT help" "MIME type synchronization"

# Test 3: Health check
run_test "health-check command" "$CLI_SCRIPT health-check" "health check"

# Test 4: Sync command exists
run_test "sync command exists" "$CLI_SCRIPT help" "sync.*Synchronize"
