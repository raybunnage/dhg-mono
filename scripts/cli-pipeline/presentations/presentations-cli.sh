...
function display_help() {
  ...
  # Key Commands Section - Based on actual usage from CLI report
  echo -e "\033[1mMOST USED COMMANDS:\033[0m"
  echo -e "  (Based on usage statistics from the CLI report)"
  echo ""
  
  echo -e "\033[1m1. health-check\033[0m - Check health of pipeline infrastructure (80 uses, 65% success)"
  echo -e "   Usage: presentations-cli health-check [options]"
  echo -e "   Options:"
  echo -e "     --skip-database             Skip database connection check"
  echo -e "     --skip-presentations        Skip presentations table check"
  echo -e "     --skip-claude               Skip Claude service check"
  ...
  echo -e "\033[1m2. find-missing-presentations\033[0m - Find folders needing presentations (61 uses, 80.3% success)"
  echo -e "   Usage: presentations-cli find-missing-presentations [options]"
  echo -e "   Options:"
  echo -e "     --create-missing            Create the missing presentations"
  echo -e "     --dry-run                   Preview changes without creating"
  echo -e "     -l, --limit <number>        Limit the number to process"
  echo -e "     -v, --verbose               Show detailed logs"
  echo ""
  
  echo -e "\033[1m3. process-mp4-files\033[0m - Process MP4 files and generate AI summaries (54 uses, 81.5% success)"
  ...
  echo -e "\033[1m4. review-presentations\033[0m - Review presentation status (35 uses, 48.6% success)"
  echo -e "   Usage: presentations-cli review-presentations [options]"
  echo -e "   Options:"
  echo -e "     -p, --presentation-id <id>   Review specific presentation"
  echo -e "     -e, --expert-id <id>         Filter by expert ID"
  echo -e "     -s, --status <status>        Filter by status"
  echo -e "     -l, --limit <number>         Limit results (default: 1000)"
  echo -e "     -f, --format <format>        Output format (table|json)"
  echo -e "     -o, --output-file <path>     Write to file"
  echo -e "     -c, --create-assets          Create missing assets"
  echo ""
  
  echo -e "\033[1m5. create-presentation-assets\033[0m - Create assets for presentations (30 uses, 86.7% success)"
  ...
  echo -e "     -l, --limit <number>         Limit presentations to process"
  echo -e "     -d, --depth <number>         Folder search depth (default: 6)"
  echo ""
  
  echo -e "\033[1mOTHER FREQUENTLY USED COMMANDS:\033[0m"
  echo -e "  • create-presentations-from-mp4    Create presentations for MP4 files (23 uses)"
  echo -e "  • generate-summary                 Generate AI summaries from transcripts (16 uses)"
  echo -e "  • check-video-consistency          Check video consistency (10 uses)"
  echo -e "  • repair-mismatched-video-ids      Fix video ID mismatches (9 uses)"
  echo ""
