...
/**
 * Create a minimal expert information object with just ID and name
 */
...
/**
 * Get a list of folders that need expert assignment
 * These are folders with path_depth = 0 and non-null main_video_id
 */
async function getFoldersNeedingExperts(): Promise<MinimalFolder[]> {
  const supabaseClientService = SupabaseClientService.getInstance();
  const supabase = supabaseClientService.getClient();
...
async function runInteractiveMode(): Promise<void> {
  ...
  try {
    // Get all experts with mnemonics at the start (we won't display them)
    ...
    let keepGoing = true;
    while (keepGoing) {
      // Get all folders that need experts assigned
      const allFolders = await getFoldersNeedingExperts();
      
      // Filter out folders we've already processed (whether assigned or skipped)
      const remainingFolders = allFolders.filter(folder => !processedFolderIds.has(folder.id));
      ...
      if (!expert) {
        loggerUtil.error(`❌ Expert with mnemonic "${mnemonicInput}" not found. Returning to this folder...`);
      ...
      loggerUtil.info(`✅ Assigned expert "${expert.name}" to folder "${currentFolder.name}"`);
    }
  } catch (error: any) {
...
/**
 * Main function for assigning experts to folders
 * Can run in interactive mode or direct assignment mode
 */
export async function assignExpert(options: AssignExpertOptions): Promise<void> {
  ...
  // Validate required parameters for direct assignment
  if (!folderId) {
    loggerUtil.error('Folder ID is required. Use --folder-id option.');
    return;
  }
  
  if (!expertId) {
    loggerUtil.error('Expert ID is required. Use --expert-id option.');
    return;
  }
