...
if [ "$COMMAND" = "add-expert" ]; then
  ...
  if [ "$COMMAND" = "help" ] || [ "$COMMAND" = "--help" ] || [ "$COMMAND" = "-h" ]; then
    ...
    echo "    assign-multiple-experts Interactively assign multiple experts to individual folders (with DELETE option)"
    ...
    echo ""
    echo "  # Assign multiple experts to individual folders interactively (with DELETE option)"
    echo "  ./experts-cli.sh assign-multiple-experts"
    echo ""
    echo "  # Process all high-level folders (no limit)"
    echo "  ./experts-cli.sh assign-multiple-experts --limit 0"
    echo ""
    echo "  # DELETE an expert from a folder using the mnemonic"
    echo "  ./experts-cli.sh assign-multiple-experts  # Then type: DELETE WAG"
    echo ""
    echo "  # Press Enter to quickly move to the next folder"
    echo "  ./experts-cli.sh assign-multiple-experts  # Then just press Enter to skip to next"
    echo ""
