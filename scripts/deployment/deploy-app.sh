...
VITE_APP_ENV=$ENV NODE_ENV=$ENV pnpm build

# Deploy
if [ "$ENV" = "production" ]; then
  netlify deploy --prod --dir="$DIST_PATH" --message "$APP_NAME: Production deployment"
else
  netlify deploy --dir="$DIST_PATH" --message "$APP_NAME: $ENV deployment"
fi 