-- Migration to fix created_by and updated_by references in expert_documents
-- Created at: 2025-02-27 18:41:42

BEGIN;

-- 1. Check if expert_documents table exists
DO $$ 
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Table expert_documents does not exist';
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE p.proname IN ('set_created_by', 'set_updated_by')
      AND t.tgrelid \!= 'expert_documents'::regclass
  ) THEN
